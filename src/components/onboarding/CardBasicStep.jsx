    ...
    */
    ...
    const handleProfileImageChange = async (event) => {
        ...
        setImageError("이미지는 5MB 이하만 등록할 수 있습니다.");
        ...
        handleChange("profileImagePreview", previewUrl);
        ...
        handleChange("profileImageUrl", uploadData.url);

        handleChange(
            "profileImagePreview",
            getProfileImagePreviewUrl(uploadData.url),
        );

        URL.revokeObjectURL(previewUrl);
        } catch (requestError) {
        console.error("프로필 이미지 업로드 실패:", requestError);

        handleChange("profileImageUrl", "");

        setImageError(requestError?.message || "이미지 업로드에 실패했습니다.");
        } finally {
